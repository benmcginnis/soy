...
export function noAutoescape(thingToPrint: any) {
    return (
        <>
            {thingToPrint}
        </>
...
export function escapeHtml(thingToPrint: any) {
    return (<>
        {ReactDOMServer.renderToStaticMarkup(thingToPrint)}
    </>
  )
}
...
export function augmentMap(map1: object, map2 :object) :object{
    return { ...map1, ...map2 }
}
...
export function strContains(haystack: string, needle :string) :boolean {
...
export function bidiEndEdge() :"right"|"left" {
    return 'right';
}
...
export function bidiSpanWrap(arg :any) {
    return (
        <span dir={'ltr'}>
            {arg}
        </span>
...
export function range(limit :number, init:number=0, increment :number=1 ) :Array<number> {
    const retValue = [];
    console.log(`init:${init}, limit:${limit}, increment:${increment}`)
